...
## 変更履歴
2017/12/26　ver 0.0.1 新規作成  
2018/03/27　ver 1.0.1 API追加（アカウント関連、フォトクロークトークン取得、フォトクロークプレフィックス取得）
...
※　通信する経路は全て暗号化するため、httpsを利用します。  
※　合言葉照合が不要なAPIへのアクセスにはフォトクロークトークンは不要です。  
※　フォトクロークトークンが不正の場合は、APIでエラーが返却されます。
```
...
※標準サイズはオーナーもしくはDL許可クロークでないと取得できない
